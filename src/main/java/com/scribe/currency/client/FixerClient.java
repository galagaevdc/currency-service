...
public class FixerClient {
    ...
    @PostConstruct
    public void init() {
        this.exchangeUrl = "%s/latest?access_key=%s".formatted(fixerApiConfig.getUrl(),
                fixerApiConfig.getKey());
        this.supportedCurrenciesUrl = "%s/symbols?access_key=%s".formatted(fixerApiConfig.getUrl(), fixerApiConfig.getKey());
    }
    ...
    public List<String> getSupportedCurrencies() {
        FixerSymbolsResponse response = restTemplate.getForObject(supportedCurrenciesUrl, FixerSymbolsResponse.class);
        processErrorResponse(response);
